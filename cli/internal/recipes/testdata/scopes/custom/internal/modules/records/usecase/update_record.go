...
// UpdateRecord changes one record when the policy lets the caller change it.
// It returns ErrRecordVersionConflict when in.Version is no longer current.
// An update that changes nothing returns the record as it is. The policy
...
func (s *Service) UpdateRecord(ctx context.Context, id string, in UpdateRecordInput) (domain.Record, error) {
	...
	err := s.store.InTx(ctx, func(tx Store) error {
		current, err := tx.SelectRecord(ctx, id, true)
		if err != nil {
			return err
		}
