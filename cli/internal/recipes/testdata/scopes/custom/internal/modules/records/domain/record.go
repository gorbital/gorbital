...
type Record struct {
	...
	// fact about a record the generator knows the policy may want. It
...
// NewRecord returns a new record created by createdBy, or a
// *ValidationError. Whether createdBy may create one is the policy's
// decision, not this function's. Text is trimmed, and an empty choice
// takes its first value.
func NewRecord(id, createdBy string, f RecordFields, now time.Time) (Record, error) {
	f.Title = strings.TrimSpace(f.Title)
	f.Note = strings.TrimSpace(f.Note)
	if f.State == "" {
		f.State = StateOpen
	}
	record := Record{ID: id, CreatedBy: createdBy, RecordFields: f, Version: 1, CreatedAt: now, UpdatedAt: now}
