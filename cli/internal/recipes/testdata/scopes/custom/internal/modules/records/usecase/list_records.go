...
// ListRecordsInput selects a page of the records the policy allows, from all records.
...
// ListRecords returns a page of the records the policy lets the caller
// see, sorted by one field (newest first by default). The narrowing
// happens in the store, which asks Policy.Filter before every list: it is
// the only thing between this request and every row in the table.
func (s *Service) ListRecords(ctx context.Context, in ListRecordsInput) (page.Result[domain.Record], error) {
