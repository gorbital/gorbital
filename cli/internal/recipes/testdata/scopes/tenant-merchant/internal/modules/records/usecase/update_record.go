...
// UpdateRecord changes one of the merchant merchantID's records. It returns
// ErrRecordVersionConflict when in.Version is no longer current. An update
// that changes nothing returns the record as it is.
func (s *Service) UpdateRecord(ctx context.Context, merchantID, id string, in UpdateRecordInput) (domain.Record, error) {
	...
	err := s.store.InTx(ctx, func(tx Store) error {
		current, err := tx.SelectRecord(ctx, merchantID, id, true)
		if err != nil {
			return err
		}
