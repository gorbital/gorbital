...
// Record belongs to one merchant. Its members reach it through their
// role; nobody else can see or change it.
type Record struct {
	ID         string
	MerchantID string
	// CreatedBy is the member who created it, a user or the merchant's
...
// NewRecord returns a new record of merchantID created by createdBy, or a
// *ValidationError. Text is trimmed, and an empty choice takes its first
// value.
func NewRecord(id, merchantID, createdBy string, f RecordFields, now time.Time) (Record, error) {
	f.Title = strings.TrimSpace(f.Title)
	f.Note = strings.TrimSpace(f.Note)
	if f.State == "" {
		f.State = StateOpen
	}
	record := Record{ID: id, MerchantID: merchantID, CreatedBy: createdBy, RecordFields: f, Version: 1, CreatedAt: now, UpdatedAt: now}
