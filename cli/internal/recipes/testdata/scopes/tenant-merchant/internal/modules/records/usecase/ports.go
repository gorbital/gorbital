...
// ListQuery selects one page of a merchant's records.
...
// Every method is limited to one merchant's records.
type Store interface {
	...
	// SelectRecord returns one of merchantID's records, or ErrRecordNotFound.
	// lock locks the row until the transaction ends.
	SelectRecord(ctx context.Context, merchantID, id string, lock bool) (domain.Record, error)
	...
	// ErrRecordNotFound.
	DeleteRecord(ctx context.Context, merchantID, id string) error
