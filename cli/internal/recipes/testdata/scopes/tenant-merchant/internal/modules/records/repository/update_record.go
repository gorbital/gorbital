...
const updateRecordSQL = `
	...
	WHERE id = $1 AND merchant_id = $2 AND version = $7
...
// no row has that version (changed, deleted or not the merchant's), and
// ErrRecordTitleTaken.
func (s *Store) UpdateRecord(ctx context.Context, record domain.Record) (domain.Record, error) {
	rows, err := s.db.Query(ctx, updateRecordSQL,
		record.ID, record.MerchantID, record.Title, record.Note, record.State, record.UpdatedAt, record.Version)
	if err != nil {
