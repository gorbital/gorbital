...
// Register adds the records routes to r, under a merchant. Every
// route requires a signed-in member of the merchant in the path whose
// role grants the permission its guard names (guard.Scope): anyone else
// gets 404 merchant_not_found, as if the merchant didn't exist. The use cases
// reach only that merchant's records.
func Register(r *gorbital.Router, svc *usecase.Service) {
	h := handlers{svc: svc}
	records := r.Group("/v1/merchants/{merchantId}/records", gorbital.Tags("Records"))
	...
	gorbital.Post(records, "", h.createRecord, gorbital.OperationID("records-create"),
		...
		guard.Scope(usecase.PermWrite))
	gorbital.Get(records, "", h.listRecords, gorbital.OperationID("records-list"),
		gorbital.Summary("List the merchant's records"),
