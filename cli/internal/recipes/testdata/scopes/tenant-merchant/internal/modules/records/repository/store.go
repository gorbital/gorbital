...
const recordColumns = `id, merchant_id, created_by, title, note, state, version, created_at, updated_at`

func scanRecord(row pgx.CollectableRow) (domain.Record, error) {
	var record domain.Record
	err := row.Scan(&record.ID, &record.MerchantID, &record.CreatedBy, &record.Title, &record.Note, &record.State, &record.Version, &record.CreatedAt, &record.UpdatedAt)
