...
// SelectRecords returns up to q.Limit of a merchant's records in q.Sort
...
func (s *Store) SelectRecords(ctx context.Context, q usecase.ListQuery) ([]domain.Record, error) {
	...
	rows, err := s.db.Query(ctx, sql, q.MerchantID, q.After != nil, after, afterID, q.Limit, q.State)
	if err != nil {
		return nil, err
	}
