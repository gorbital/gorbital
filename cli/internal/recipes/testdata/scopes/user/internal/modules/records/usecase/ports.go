...
// ListQuery selects one page of an owner's records.
...
// Every method is limited to one owner's records.
type Store interface {
	...
	// SelectRecord returns one of ownerID's records, or ErrRecordNotFound.
	// lock locks the row until the transaction ends.
	SelectRecord(ctx context.Context, ownerID, id string, lock bool) (domain.Record, error)
