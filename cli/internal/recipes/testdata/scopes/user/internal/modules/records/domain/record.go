...
// Record belongs to one user, its owner, and only the owner can see or
// change it.
type Record struct {
	ID      string
	OwnerID string
...
// NewRecord returns a new record owned by ownerID, or a *ValidationError.
// Text is trimmed, and an empty choice takes its first value.
func NewRecord(id, ownerID string, f RecordFields, now time.Time) (Record, error) {
	f.Title = strings.TrimSpace(f.Title)
	f.Note = strings.TrimSpace(f.Note)
	if f.State == "" {
		f.State = StateOpen
	}
	record := Record{ID: id, OwnerID: ownerID, RecordFields: f, Version: 1, CreatedAt: now, UpdatedAt: now}
