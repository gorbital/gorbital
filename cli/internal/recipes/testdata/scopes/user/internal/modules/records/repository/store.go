...
const recordColumns = `id, owner_id, title, note, state, version, created_at, updated_at`

func scanRecord(row pgx.CollectableRow) (domain.Record, error) {
	var record domain.Record
	err := row.Scan(&record.ID, &record.OwnerID, &record.Title, &record.Note, &record.State, &record.Version, &record.CreatedAt, &record.UpdatedAt)
