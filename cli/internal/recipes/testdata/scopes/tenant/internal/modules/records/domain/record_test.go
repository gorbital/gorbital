...
func TestNewRecord(t *testing.T) {
	...
	f.Note = "  " + f.Note + "  "
	f.State = ""
	record, err := domain.NewRecord("rcr_1", "org_1", "usr_1", f, now)
	if err != nil {
		t.Fatal(err)
	}
	if record.RecordFields != validFields() || record.ID != "rcr_1" || record.OrgID != "org_1" || record.CreatedBy != "usr_1" || record.Version != 1 ||
...
func TestNewRecordValidates(t *testing.T) {
	...
	for _, tt := range tests {
		f := validFields()
		tt.edit(&f)
		_, err := domain.NewRecord("rcr_1", "org_1", "usr_1", f, now)
...
func TestApply(t *testing.T) {
	record, err := domain.NewRecord("rcr_1", "org_1", "usr_1", validFields(), now)
	if err != nil {
		t.Fatal(err)
	}
