...
// Register adds the records routes to r, under an organisation. Every
...
// reach only that organisation's records.
func Register(r *gorbital.Router, svc *usecase.Service) {
	h := handlers{svc: svc}
	records := r.Group("/v1/orgs/{orgId}/records", gorbital.Tags("Records"))
	...
	gorbital.Get(records, "", h.listRecords, gorbital.OperationID("records-list"),
		gorbital.Summary("List the organisation's records"),
