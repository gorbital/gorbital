// Package records is the records module: records that belong to an
...
func Module() gorbital.Module {
	return gorbital.Module{
		...
		Errors: []httpx.Mapping{
			...
			{Err: domain.ErrRecordNotFound, Status: http.StatusNotFound, Code: "record_not_found", Detail: "the organisation has no record with this ID"},
			{Err: domain.ErrRecordTitleTaken, Status: http.StatusConflict, Code: "record_title_taken", Detail: "the organisation already has a record with this title"},
		...
		Permissions: []gorbital.Permission{
			{Name: usecase.PermRead, Description: "See the organisation's records", OrgRoles: []string{"owner", "admin", "member"}},
			{Name: usecase.PermWrite, Description: "Create, change and delete the organisation's records", OrgRoles: []string{"owner", "admin", "member"}},
		},
