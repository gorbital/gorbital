...
// UpdateRecord changes one of the organisation orgID's records. It returns
// ErrRecordVersionConflict when in.Version is no longer current. An update
// that changes nothing returns the record as it is.
func (s *Service) UpdateRecord(ctx context.Context, orgID, id string, in UpdateRecordInput) (domain.Record, error) {
	...
	err := s.store.InTx(ctx, func(tx Store) error {
		current, err := tx.SelectRecord(ctx, orgID, id, true)
		if err != nil {
			return err
		}
