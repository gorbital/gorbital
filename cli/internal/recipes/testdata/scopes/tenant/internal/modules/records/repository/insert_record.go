...
const insertRecordSQL = `
	INSERT INTO records (id, org_id, created_by, title, note, state, version, created_at, updated_at)
...
func (s *Store) InsertRecord(ctx context.Context, record domain.Record) (domain.Record, error) {
	rows, err := s.db.Query(ctx, insertRecordSQL,
		record.ID, record.OrgID, record.CreatedBy, record.Title, record.Note, record.State, record.Version, record.CreatedAt, record.UpdatedAt)
	if err == nil {
