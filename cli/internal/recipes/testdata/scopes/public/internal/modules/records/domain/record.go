...
// Record is world-readable: anyone can read one, and only a caller with
...
// NewRecord returns a new record, or a *ValidationError. Text is
// trimmed, and an empty choice takes its first value.
func NewRecord(id string, f RecordFields, now time.Time) (Record, error) {
	f.Title = strings.TrimSpace(f.Title)
	f.Note = strings.TrimSpace(f.Note)
	if f.State == "" {
		f.State = StateOpen
	}
	record := Record{ID: id, RecordFields: f, Version: 1, CreatedAt: now, UpdatedAt: now}
