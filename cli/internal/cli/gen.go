...
const genUsage = `Usage:
  ...
  orb gen middleware <Name> (--module <name> [--guard] | --global) [flags]
  orb gen modules [flags]
  orb gen job <Name> [flags]
  orb gen resource <Name> <field:type>... [flags]
  orb gen migration <name> [flags]

In an app on gorbital.Main: module generates a layered module (domain,
use cases, repository, routes, one file per operation) with its table,
tests and permissions; middleware generates middleware or a guard with its
test; modules rewrites internal/modules/modules.gen.go, the list of the app's
modules. In an app created with orb v0.1's Full preset: job generates a
background job whose schedule, timeout and retries can be changed at runtime
through /ops/jobs; resource generates a module, table and API for records
that belong to the signed-in user (in an app on gorbital.Main it runs orb gen
module). migration creates an empty database migration that runs after the
existing ones, in either. In a terminal, missing values are asked
...
func runGen(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
	if len(args) == 0 || strings.HasPrefix(args[0], "-") {
		...
		return usageError("missing generator: orb gen module <Name> <field:type>..., orb gen middleware <Name>, orb gen modules, orb gen job <Name>, orb gen resource <Name> <field:type>... or orb gen migration <name>")
	}
	switch args[0] {
	case "job":
	...
	case "module":
		return runGenModule(ctx, args[1:], stdin, stdout, stderr)
	case "middleware":
		return runGenMiddleware(ctx, args[1:], stdin, stdout, stderr)
	case "migration":
		return runGenMigration(ctx, args[1:], stdin, stdout, stderr)
	case "modules":
		return runGenModules(ctx, args[1:], stdout, stderr)
	default:
		return usageError(fmt.Sprintf("unknown generator %q (want module, middleware, modules, job, resource or migration)", args[0]))
	}
}
