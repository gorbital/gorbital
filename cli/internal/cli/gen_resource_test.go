...
func TestGenResourceValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{"org scope without organisations", []string{"gen", "resource", "Project", "name:string", "--scope", "org"}, 1, "--scope tenant needs the app's tenancy"},
		{"tenant scope without organisations", []string{"gen", "resource", "Project", "name:string", "--scope", "tenant"}, 1, "--scope tenant needs the app's tenancy"},
		{"public scope in a v0.1 app", []string{"gen", "resource", "Project", "name:string", "--scope", "public"}, 2, "--scope public needs an app on gorbital.Main"},
		{"custom scope in a v0.1 app", []string{"gen", "resource", "Project", "name:string", "--scope", "custom"}, 2, "--scope custom needs an app on gorbital.Main"},
