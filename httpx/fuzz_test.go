...
// FuzzIPFilter parses allow and deny lists and filters a client address:
// nothing panics, accepted lists are canonical, and the decision matches a
// direct reading of the rules.
func FuzzIPFilter(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, allowList, denyList, remote string) {
		allow, errA := httpx.ParsePrefixes(allowList)
		deny, errD := httpx.ParsePrefixes(denyList)
		...
		mw, err := httpx.IPFilter(allow, deny)
		if err != nil {
			return
		}
		called := false
...
func FuzzParsePrefixes(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, list string) {
		got, err := httpx.ParsePrefixes(list)
		if err != nil {
			if got != nil {
		...
		again, err := httpx.ParsePrefixes(strings.Join(strs, ","))
		if err != nil || !slices.Equal(again, got) {
