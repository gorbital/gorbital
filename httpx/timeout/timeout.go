// Package timeout gives each request a deadline and answers 503
// request_timeout when the handler hasn't started its response by then
// (ADR-0085):
//
...
//	)
//
// gorbital.New installs it as the Timeout step of the default middleware
// stack, configured by APP_REQUEST_TIMEOUT, and gorbital.Timeout shortens it
// for one route.
//
...
// New returns middleware that gives each request a context deadline d from
// now. When the
...
// httpx.Recover and httpx.RequestID.
...
func (tw *timeoutWriter) answerLocked() {
	...
	tw.answered = true
	p := httpx.NewProblem(http.StatusServiceUnavailable, "request_timeout", "the request took too long; try again later")
