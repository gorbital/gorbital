...
- Security layers ([ADR-0085](docs/adr/0085-security-layers.md), guide: [Security layers](docs/guides/security-layers.md)):
  - `httpx.Timeout(d)`: a context deadline per request and, when the handler hasn't started its response, 503 with the new error code `request_timeout`; later writes are discarded, streaming and `http.ResponseController` keep working.
