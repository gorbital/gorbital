...
- Security layers ([ADR-0085](docs/adr/0085-security-layers.md), guide: [Security layers](docs/guides/security-layers.md)):
  - `httpx.Timeout(d)`: a context deadline per request and, when the handler hasn't started its response, 503 with the new error code `request_timeout`; later writes are discarded, streaming and `http.ResponseController` keep working. Apps on `gorbital.Main` get it as the `Timeout` step of the default stack (`APP_REQUEST_TIMEOUT`, default 30s), and `gorbital.Timeout(d)` shortens it for a route.
  - `httpx.IPFilter(allow, deny)` and `httpx.ParsePrefixes`: refuse requests by client address after `TrustedProxies` with the new error code `ip_not_allowed` (403); `httpx.ErrDenyAll`.
