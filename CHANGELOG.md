...
- Sign-in options, hooks and custom methods in `gorbital.dev/gorbital/authhttp` ([ADR-0083](docs/adr/0083-modules-stack-migrations-and-ejection.md#phase-6-implementation-notes-sign-in-options-hooks-and-custom-methods-2026-09-17), Phase 6). `New(opts ...Option)`: without options it is v0.1's sign-in, and the frozen v0.1.0 OpenAPI of `/v1/auth/*` is unchanged. Invalid options are reported by `CheckConfig` (exit status 2) and `Setup`.
  ...
  - `RegisterFields[T]`: fields of the app's on `POST /v1/auth/register`, validated by Huma from `T`'s tags and `Resolve` before anything is stored, shown in OpenAPI, and saved in the account's transaction.
  ...
  - Shelfie chapters [6. Accounts](docs/examples/shelfie/06-accounts.md) (registration fields into a profiles module, a default shelf, suspended readers) and [7. Phone-code sign-in](docs/examples/shelfie/07-phone-code-sign-in.md) (a `phonelogin` module through `SignIn`).
