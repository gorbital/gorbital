...
- Shelfie, the example app of the Examples tab (`examples/apps/shelfie`), written chapter by chapter from [Start a project](docs/examples/shelfie/00-start-a-project.md) through a books module, tests, operations, accounts, a phone-code sign-in method of its own, book clubs and the generators. Its modules use four layers with one file per operation ([ADR-0083](docs/adr/0083-modules-stack-migrations-and-ejection.md#3-app-layout)).
- Recipes with their own apps in `examples/apps`: [Multi-tenant invoicing](docs/examples/recipes/multi-tenant-invoicing.md) (companies as organisations, an invoices module from `orb gen module --org`, row-level security tested as a database role without `BYPASSRLS`), [Internal admin tool](docs/examples/recipes/internal-admin-tool.md) and [Upgrading a v0.1 app](docs/examples/recipes/upgrading-a-v0.1-app.md).
