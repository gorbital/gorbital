...
- Sign-in options, hooks and custom methods in `gorbital.dev/gorbital/authhttp` ([ADR-0083](docs/adr/0083-modules-stack-migrations-and-ejection.md), Phase 6). `New(opts ...Option)`: without options it is v0.1's sign-in, and the frozen v0.1.0 OpenAPI of `/v1/auth/*` is unchanged. Invalid options are reported by `CheckConfig` (exit status 2) and `Setup`.
