...
func (s *Store) Run(ctx context.Context) error {
	...
	for {
		...
		s.logger.WarnContext(ctx, "flags listener disconnected; reconnecting", "err", err, "retry_in", delay.String())
...
func (s *Store) listen(ctx context.Context) (bool, error) {
	...
	for {
		...
		if err := s.reloadKey(ctx, n.Payload); err != nil {
			s.logger.WarnContext(ctx, "reload changed flag", "flag", n.Payload, "err", err)
		}
	}
}
	...
	for {
		...
		case <-ticker.C:
			if err := s.Reload(ctx); err != nil && ctx.Err() == nil {
				s.logger.WarnContext(ctx, "periodic flags reload failed", "err", err)
			}
		}
	}
}
