...
├── api/{openapi.yaml, postman_collection.json, llms.txt}
├── internal/
...
- `repository/` implements the ports (sqlc, pgx); `delivery/` uses generated API types and never imports `repository/`.
- Modules never import other modules.
...
### 7.5 API contract and docs ([ADR-0027](adr/0027-api-contract-and-docs.md), pending spike)

`api/openapi.yaml` is the source of truth. Scalar serves `/docs`. The Postman collection and `llms.txt` are generated from the spec. Spec-first (oapi-codegen) is proposed; code-first (Huma) is compared in a spike.
...
- **Recipes** are declarative: `createFile`, `insertLine@anchor`, `addRequire`, `copyMigration`, `appendEnv`, `mergeOpenAPI`. No code runs at install time.
...
| Item | Resolved by |
|---|---|
| API contract: spec-first vs code-first | OpenAPI spike → ADR-0027 accepted or revised |
| Anchor edits: text insertion vs AST | Anchor-edit spike → note in ADR-0021 |
