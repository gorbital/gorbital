...
| **Where** | `modules/postgres` (pool, transactions, migrations), every repository, `modules/settings`, `modules/jobs`, `modules/auditpg`, `modules/releases`, `modules/ratelimitpg` (rate limits shared by every instance, in an unlogged table), `modules/idempotency` (responses to retried POST and PATCH requests), `modules/mail/suppressionpg` (the email suppression list) |
