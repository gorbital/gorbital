...
`.github/workflows/ci.yml` runs the jobs above on Go 1.26 and 1.27 with PostgreSQL and Mailpit service containers and `GORBITAL_REQUIRE_DB=1`: tests with `-race` for every module, golangci-lint, recipe and OpenAPI drift, end-to-end generation, govulncheck and gitleaks. The workflows are currently disabled on GitHub during active development, so run the commands locally ([local development](local-development.md#ci)).
