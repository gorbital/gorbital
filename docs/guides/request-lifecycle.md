...
| 6 | `httpx.AccessLog` | core `httpx` | After the response, one `http request` log line with `source=http`, `method`, `path`, `route`, `status`, `duration_ms`, `bytes`, `request_id`, `trace_id`, `span_id`, and what later middleware noted with `httpx.AccessNoteFrom(ctx).Add(…)`: `user_id` from `auth.Middleware` ([ADR-0072](../adr/0072-local-log-store.md)) | — |
...
```text
level=INFO msg="http request" service=acme-api source=http method=POST path=/v1/projects route="/v1/projects" status=201 duration_ms=4 bytes=180 request_id=req_… user_id=usr_… trace_id=… span_id=…
```
