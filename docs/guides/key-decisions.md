...
v0.2.0 moved those flows into the library: `gorbital.dev/gorbital/authhttp`, added in one line of `main.go`, with nothing generated ([ADR-0083](../adr/0083-modules-stack-migrations-and-ejection.md)). v0.2.1 moved them back, and the line settled where it is now: `orb new` writes the flows into `internal/modules/auth` and `internal/modules/orgs` as the app's own code, while the primitives stay in the library permanently ([ADR-0092](../adr/0092-what-the-framework-owns.md), [The code in your repo](the-code-in-your-repo.md)). Most changes are still cheaper as options and hooks ([configuring sign-in](configuring-sign-in.md), [sign-in hooks](sign-in-hooks.md)), which survive upgrades untouched; the rest is a file in the app.

v0.3.0 removed the `orb eject` command that used to make this a per-module choice. Once `orb new` writes the flows into every app, an on-demand copy answers a question nobody has: its only remaining users were people undoing a default, or taking `opshttp`, `flagshttp` and `mailevents`, none of which hold business rules. One shape of Full app is cheaper to document, test and upgrade than two, and no command that can move a security primitive into an app is a structural promise rather than a guideline. The copy machinery stayed, as what `orb new`, `orb add orgs` and `orb upgrade --layout v0.2` run.

**Alternatives.** Everything in the library behind options (ADR-0024 as first written); everything generated.

**Trade-offs.** With the flows in the app, developers can read and change every one, at the price of a fix reaching them through a diff they apply rather than through `go get`, and more code to own. With them in the library it is the other way round. The price of choosing once, for everybody, is that an app wanting the library's bargain back has to unpick the copy by hand; the price of the per-module choice was two shapes of every app.
