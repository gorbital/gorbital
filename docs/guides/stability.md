...
A code is read from the source, so it has to be written where the source shows it: as a string literal in `httpx.Mapping{Code: "…"}` or `httpx.NewProblem`, or as its own package's string constant. A code taken from a struct field is invisible, which is how a library option that renames a code works — `orgshttp.ScopeName` renames `org_not_found`, so `orgshttp`'s mapping uses `gorbital.Scope.NotFoundCode`. When you add one of those, declare the default refusal too (`errOrgNotFound` in `gorbital/scope.go`), or the code leaves the reference while the route still returns it.
