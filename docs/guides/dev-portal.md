...
| `POST /_portal/api/db/ddl/plan` and `apply` | `{"change": {"kind": "add_column", "schema": "public", "table": "projects", "column": {"name": "phone", "type": "text"}}, "name": "add_phone", "allow_dirty": false}`. Kinds: create_table, drop_table, rename_table, add_column, drop_column, rename_column, alter_column, add_foreign_key, add_unique, add_check, drop_constraint, set_primary_key, create_index, drop_index, create_enum, add_enum_value, rename_enum_value, comment, rls. `plan` answers the SQL (Up, Down, notes) and the migration file it becomes; `apply` writes the file under `db/migrations` and applies it as `orb dev` does. Managed and system tables are refused (403 `system_table`) |
