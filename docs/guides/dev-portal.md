...
| `POST /_portal/api/app/migrate-down`, `migrate-redo` | Roll back the most recent migration (`go run ./cmd/migrate --down`), or roll it back and apply it again (`--redo`): the check that its Down works. Development only; 202 through the supervisor |
...
| Generators | Every generator with a form and a diff preview before it writes: `job`, `resource`, `migration`, `add-mail`, `add-storage`, `add-rls`, and `add-orgs` (its dry run, then its branch workflow) ([ADR-0077](../adr/0077-generators-hub-first-run-and-project-settings.md)) | `/_portal/api/generators/…` |
