...
| Endpoint | Returns |
|---|---|
| `GET /_portal/api/status` | `portal` (orb version, whether a UI is bundled, start time), `project` (name, module, preset, `auth` and `scope` as `orb new` recorded them — or `tenancy` for an app created before v0.3.0 — features, mail provider, directory, whether it has a database), `app` (below), `links` (`api`, `docs`, and `mail`, `console`, `grafana` when they apply), `generators` (names) |
...
| Project Settings | The app as the manifest and `.env` describe it: name, module, preset, ports, database, mail delivery, storage driver, CORS origins, logging, docs, each with the key to edit in the Environment screen; API and service-account keys from the ops API; a danger zone: reset the database (local databases only; `orb dev --fresh` does the same from the terminal), clear the log store, the inbox and the SQL history, each confirmed with what it loses | `/_portal/api/project`, `/ops/service-accounts…` |
