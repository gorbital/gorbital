...
| Change what a built-in module does | Edit `internal/modules/auth` or `internal/modules/orgs` — they are yours | You maintain the change forever, including against its security fixes |

Plateful uses the first three for its own code. The fourth needs no command and no permission: `orb new` wrote sign-in into `internal/modules/auth` and, because Plateful is multi-tenant, organisations into `internal/modules/orgs`. Every line is there to read and to change.

Owning code has one cost, and it is smaller than it sounds. What is in your repository is the flows — handlers, use cases, repositories and migrations. The primitives they call stay in the library, permanently: password hashing, session tokens, TOTP, passkey and OAuth verification, and the organisation authorisation rules. Fixes to those reach you with `go get`; a fix to a flow you own does not. `orb doctor` reports each built-in module your app owns and warns when the library's version of it has changed, quoting the changelog, so you hear about a fix and apply it yourself. The remaining built-in modules — flags, email events and `/ops` — are the library's and stay there: they are plumbing rather than business rules, so an option, a guard or a module of your own beside them is the way to bend those. Where the line falls, and why, is [ADR-0092](../adr/0092-what-the-framework-owns.md); the details are [The code in your repo](../guides/the-code-in-your-repo.md).
