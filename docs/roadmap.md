...
| [v1.1](#v11-operations-and-integrations) | Operations and integrations | 🔨 In progress | Per-organisation settings, feature flags, API keys and service accounts, GitHub sign-in, idempotency keys, Resend bounce and complaint webhooks, Prometheus `/metrics`, live observability and incidents |
...
**Status: in progress.** Plan accepted on 2026-09-16. APIs only: the Dev Portal and Observability Portal in gorbital-dashboards stay on mock data for now. Each feature lands in the library, both Full golden apps and their templates, with an upgrade note. What was built so far:
