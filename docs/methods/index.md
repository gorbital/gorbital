...
*Since* names the first release whose API listing (`api/*.txt`) has the identifier; `v0.4.0 (unreleased)` marks API added on this branch. What the tiers promise: [Stability](../guides/stability.md).
...
| [`gorbital.dev/config`](config.md) | Package config provides configuration helpers for the composition root of a gorbital app: a Secret type that never leaks into logs or output, and environment lookup with *_FILE support for mounted secrets. |
...
| [`gorbital.dev/gorbital/operation`](gorbital-operation.md) | Package operation registers operations declared as huma.Operation values, as v0.1 apps declare them for huma.Register, on a gorbital.Router. |
