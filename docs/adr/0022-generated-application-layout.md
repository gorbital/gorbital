...
│   ├── repository/          adapters implementing ports (sqlc, pgx, apistock library)
│   └── delivery/            HTTP adapter: generated OpenAPI types ↔ use cases
├── jobs/  emails/  wellknown/
├── api/                     GENERATED from api/openapi.yaml
...
| # | Rule | Prevents |
|---|---|---|
| 1 | Domain structs have no `json` or `db` tags; delivery maps to OpenAPI types, repository maps to sqlc types | API contract changing when domain or schema changes |
...
- `api/openapi.yaml` (owned) plus generated `postman_collection.json` and `llms.txt`.
- `db/migrations` (single ordered history), `db/queries` (sqlc).
