...
`orb new` offers three shapes, and each shape is a directory of templates generated from a golden application. That worked while there were three. v0.3 gives the developer two independent choices — how much sign-in (ADR-0089) and what a tenant is (ADR-0088) — and nine of their combinations are legal. Extending the present design by one directory per shape would mean nine template trees and nine golden applications.
