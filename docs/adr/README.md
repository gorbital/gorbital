...
| [0015](0015-public-api-and-stability-tiers.md) | Public API surface and stability tiers | Accepted, amended by 0054, 0092 |
...
| [0023](0023-tenancy.md) | Tenancy | Accepted, amended by 0033, 0048, 0061, 0088 |
...
| [0083](0083-modules-stack-migrations-and-ejection.md) | Modules, the default stack, migrations and ejection: `gorbital.Module` and `Deps`; built-in modules in `gorbital.dev/gorbital/…http`; app layout with generated `internal/modules/modules.gen.go`; the named default stack; `Authenticator`; one goose history with a frozen version table for library migrations; `orb eject` | Accepted, amended by 0092 |
| [0084](0084-versioned-documentation.md) | Versioned documentation: `release/v0.1` frozen and served at `/v0.1/`, the latest unprefixed, the unreleased line at `/next/`, from git refs; generated Methods pages checked in CI; Examples as runnable apps included by marker | Accepted, amended by 0093 |
...
| [0088](0088-scope-tenancy-as-a-contract.md) | Scope — tenancy as a contract: `gorbital.Scope` (name, path parameter, refusal code, `ValidID`, roles, database session) and a one-method `ScopeAuthorizer`; `guard.Scope` replaces `guard.OrgMember`; `Permission.ScopeRoles`; `postgres.WithScope`; `gorbital` stops importing `modules/orgs`; `Org*` kept as deprecated aliases and `actor.Actor.OrgID` and `gorbital.org_id` keep their names | Proposed |
| [0089](0089-sign-in-profiles.md) | Sign-in profiles: `authhttp.Methods` gates routes, settings, jobs, limiters, permissions and the OpenAPI document by method (password 13, operators 16, API keys 19, social 12, passkeys 9, TOTP 5, 74 together); migrations are not gated, so a method can be turned on later without one; `--auth none\|basic\|full`; external identity deferred to v0.4 | Proposed |
| [0090](0090-composing-presets.md) | Composing presets: `recipes.Profile` and nine legal combinations in `LegalProfiles()`; one v0.3.0 template tree with a `manifest.yaml` mapping each path to the features that must be on, a path without an entry failing the build; three byte-golden apps plus a build-and-test matrix for the rest; the profile recorded in `gorbital.yaml` and `gorbital.lock` | Proposed |
| [0091](0091-resource-access-policies.md) | Resource access policies: `orb gen resource --scope user\|tenant\|public\|custom`; `custom` generates a `policy.go` the app owns with no implicit tenant filter and no tenant column; what a role means is code and who holds it is data; per-user grants as a documented extension; generated isolation tests and an `orb routes` scope column | Proposed |
| [0092](0092-what-the-framework-owns.md) | What the framework owns: three layers — protocol and crypto in the library permanently, flows in the app, policy only in the app; `orb eject` and `--no-eject` removed and the copy planner kept internal; `opshttp`, `flagshttp` and `mailevents` become library-only; `orb doctor --security` gives copied code a fix path | Proposed |
| [0093](0093-the-examples-repository.md) | The examples repository: five of the six apps under `examples/apps` move to `github.com/gorbital/examples` with `git subtree split`, top-level application directories and lockstep tags, while the golden apps stay because they generate the CLI's templates and `shelfie` stays at `examples/shelfie` because the CLI's tests compare `orb gen module`'s output with it; apps build against the published modules; documentation includes resolve from a pinned tag | Proposed |
