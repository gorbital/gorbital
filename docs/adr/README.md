...
| [0093](0093-the-examples-repository.md) | The examples repository: `examples/apps` moves to `github.com/gorbital/examples` with `git subtree split`, top-level application directories and lockstep tags, while the golden apps stay because they generate the CLI's templates; apps build against the published modules; documentation includes resolve from a pinned tag | Proposed |
