...
| 27 | Tampered or wrong earlier release used as an upgrade's merge base, smuggling code into "unchanged" files | Earlier templates come only from a git tag or commit of an gorbital checkout or from the module proxy with checksum verification on; rendered as text, never compiled or run; every rebuilt file checked against its hash in `gorbital.lock`; files that don't match are compared 2-way and can only conflict ([ADR-0050](0050-upgrades-and-adding-features.md)) | v0.5 |
...
| 32 | Client IP spoofing through forwarded headers to escape per-IP limits or falsify audit addresses; one load balancer's address sharing a limit for every client | `X-Forwarded-For` read only from configured trusted proxies (`APP_TRUSTED_PROXIES`), right to left; all-trusting ranges refused; untrusted peers keep their own address | v1.0 |
| 31 | Operational endpoints or tools leaking secrets or infrastructure detail | `GET /ops/system` reports no connection strings, dependency host names, environment variables or settings values, and fixed error descriptions instead of driver messages; `orb doctor` never prints `.env` values | v0.5 |
...
| 28 Upgrade drops edits | Done: `cli/internal/merge` keeps every line the developer added (`TestPlan`); unproven bases can't take the template silently; dirty trees are refused, conflicts exit 1 uncommitted on the branch; an app edited with `orb` at `v0.4.0` upgrades with every edit kept (`TestUpgradeFromV040`) |
