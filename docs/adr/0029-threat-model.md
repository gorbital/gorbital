...
| 31 | Operational endpoints or tools leaking secrets or infrastructure detail | `GET /ops/system` reports no connection strings, dependency host names, environment variables or settings values, and fixed error descriptions instead of driver messages; `aps doctor` never prints `.env` values | v0.5 |
