...
The internal security review of September 2026 found that the reason rule of threat 24 could be sidestepped and that retry could re-run finished work (OPS-2, OPS-3), and that recipients' addresses could reach job errors and logs (OPS-5).

| Finding | Change | Why this shape |
|---|---|---|
| OPS-2: a 1-second timeout, `max_attempts: 1` or a queue move stops a job without a reason | `Manager.Update` and `Reset` need a reason whenever the timeout, max attempts or queue change, besides disabling and rescheduling (`needsReason`). Enabling and priority changes don't | Each of these can stop a job doing its work while it still shows as enabled on schedule. A reason is cheap for operators and lands in the history |
| OPS-2: queue pause without a reason | `PauseQueueWithReason(ctx, name, reason)` needs a non-blank reason and records it in `jobs.queue.paused` metadata; `ResumeQueueWithReason` records an optional one. `PauseQueue` stays, deprecated, and returns `ErrReasonRequired`; `ResumeQueue` is unchanged. `POST /ops/queues/{name}/pause` takes an optional body `{reason}` and answers 422 `job_reason_required` without one | The Go API is frozen (ADR-0054), so the reason arrives through new methods rather than a changed signature; an optional request body keeps `/ops` compatible (`TestOpsAPICompatible`) |
| OPS-2: unlimited run-now | `RunNow` refuses with `ErrRunLimited` (429 `job_run_limited`) while the newest job of the definition is available, pending, retryable or running, or was created less than `MinScheduleInterval` ago; the insert carries `UniqueOpts{ByState: available, pending, retryable, running, scheduled}` so concurrent requests queue one run. Disabled definitions were already refused | The same one-minute floor as schedules, with no new setting. The check before the insert gives the time floor; River's unique insert closes the race between concurrent requests. Periodic runs aren't unique jobs, so the check, not the unique key, covers overlap with them |
| OPS-3: retry of completed runs and of disabled definitions | `Retry` locks the job (`SELECT kind, state FROM river_job … FOR UPDATE`, `select_job_for_retry.go`), accepts only `retryable`, `discarded` and `cancelled` jobs (`ErrJobNotRetryable`, 409 `job_not_retryable`) of enabled or undefined kinds (`ErrDefinitionDisabled`), then calls `JobRetryTx` in the same transaction | River's `JobRetry` makes any non-running job available again. `retryable` stays allowed: retrying it only brings its next attempt forward. A completed `gorbital.mail.send` job would otherwise deliver the email again |
| OPS-5: addresses in job errors and logs | The mail worker wraps every error it returns in a `redactedError` whose text has addresses replaced by `[email]` (`mail.RedactAddresses`) and which still unwraps to the original, so `errors.Is(err, mail.ErrRejected)` and `river.JobCancel` keep working. The error handler logs redacted text, and River's own logger is wrapped in a redacting `slog.Handler` (River logs attempt errors itself) | Redacting where text leaves the process covers every provider, including community ones; job errors of application workers are the application's responsibility and aren't rewritten |

| Check | Result |
|---|---|
| `TestUpdateRequiresReasonForLimits`, `TestRunNowIsLimited` (sequential and 10 concurrent requests), `TestRetryRefusesCompletedJobsAndDisabledDefinitions`, `TestPauseAndResumeQueue` (reason required and recorded) | Pass |
| `TestMailWorkerCancelsRejectedEmail` (stored error has `[email]`, not the address), `TestFailedAttemptsAreLoggedOnce` (no address anywhere in the log, River's lines included) | Pass |
| `TestJobsThroughOps` in both golden apps: 422 for a timeout change and a pause without reason, 429 for a second run, 409 for retrying a completed run, reason in `jobs.queue.paused` | Pass |
| `go run -C internal/tools/apicheck .` | Additions only: `PauseQueueWithReason`, `ResumeQueueWithReason`, `ErrRunLimited`, `ErrJobNotRetryable` |
