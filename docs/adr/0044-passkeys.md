...
| `auth_webauthn_ceremonies` | `token_hash` (unique), `user_id` (NULL for passwordless sign-in), `purpose` (`register`, `login`, `second_factor`), `mfa_challenge_id`, `session_data` (the library's state as JSON), `expires_at` (5 minutes), `consumed_at` |

### Registering a passkey

| Endpoint | Behaviour |
|---|---|
| `POST /v1/auth/passkeys/registration` | Signed in. Body carries the password, unless the session is verified with a second factor. Returns a `ceremony_token` and the `options` for `navigator.credentials.create()`: resident key required, user verification required, attestation `none`, the user's existing passkeys excluded |
...
| `DELETE /v1/auth/passkeys/{id}` | Needs a session verified with a second factor, or the password. Refused with 409 `mfa_required_by_role` when it's the user's last second factor and a role requires one. Emails "passkey removed" |
...
- Turning the authenticator app off keeps the recovery codes while a passkey remains; removing the last second factor deletes them. Accounts with only passkeys replace recovery codes from a session verified with a passkey.
- Account deletion and turning the authenticator app off take a code or recovery code; a passkey answers only sign-in challenges, where its ceremony is bound to the challenge.
- `auth_cleanup` removes ceremonies a day after they expire.
