...
**Decision:** (c). Constructor + small optional interfaces at runtime; `recipe/gorbital-module.yaml` for tooling; capability-based `needs/provides`.
...
**Consequences:** The manifest schema becomes a public contract that must be versioned (`apiVersion: gorbital.dev/v1`).
