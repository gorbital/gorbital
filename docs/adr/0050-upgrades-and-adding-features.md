...
| Finding | Fix | Check |
|---|---|---|
| CLI-3: the checksum policy missed `GONOSUMDB`, `GOPRIVATE` and `GOINSECURE` patterns with a trailing slash (`gorbital.dev/`), which Go honours, so an unverified module download could become the merge base | `matchesModulePrefix` follows `golang.org/x/mod/module.MatchPrefixPatterns`: one trailing slash is dropped and a pattern of n elements matches the module path's first n. Surrounding spaces are still trimmed, so orb refuses in more cases than Go, never fewer. `GOSUMDB` must be unset, `sum.golang.org` or `sum.golang.google.cn` (optionally with a proxy URL); another database or key is refused | `TestMatchesModulePrefix` (16 patterns, each checked against Go's function), `TestChecksumPolicy` |
| CLI-4: `orb upgrade` and `orb add orgs` rendered `inputs.name` and `inputs.module` from `gorbital.lock` or `gorbital.yaml` without the checks `orb new` applies. A module path carrying Go syntax rendered into every file the operation creates | `readLock` (v2) and `readManifest` check the name and module with `validateName` and `validateModule`, the preset and tenancy with `recipes.LookupPreset`, and the mail provider against `resend` and `smtp`. `orb doctor` reports an invalid file through the same readers | `TestReadLockRejects` (injected module, newline in name, unknown preset, tenancy or provider, no inputs), `TestReadManifestRejectsInvalidInputs` |
| CLI-5: the merge base could come from a "checkout" committed inside the app (named by the app's `replace` directive), and git run in a subdirectory reads the enclosing app repository. With a tampered lock, an auto-committed upgrade could revert or delete developer files | `releaseFromCheckout` requires the checkout to be the top of its own git work tree and the release commit's `go.mod` to be `module gorbital.dev` (or `apistock.dev` before the rename). `releaseCheckout` refuses `--local` inside the app's git work tree and passes over a detected checkout there (the `replace` directive or the checkout the command runs in), so the release then comes from the verified module proxy | `TestReleaseCheckoutRefusesCheckoutInsideApp`, `TestReleaseFromCheckoutNeedsAGorbitalCommit`, `TestReleaseFromCheckout` |
| End to end | `TestUpgradeFromV040` built `orb` at `v0.4.0`, but every tag up to `v0.5.0` predates the rename: the CLI is `cmd/aps` and the module `apistock.dev`, so it could no longer run. It is now `TestUpgradeFromRelease`, which upgrades from `ORB_UPGRADE_FROM` or else the newest `vX.Y.Z` tag whose `go.mod` is `module gorbital.dev` with `cli/cmd/orb`. It skips with that reason while no such tag exists, and skips when that release writes the same files as this `orb`. The routes check now requires the developer's line and no conflict markers, since the template change it looked for was specific to `v0.4.0` | `ORB_E2E=1` on 2026-09-16: skips (no post-rename tag). With `ORB_UPGRADE_FROM=1c5f6d9` (the rename commit) it passes in 45 s: every edit kept, v2 lock recording SMTP, upgrade committed, no newly failing app test. With `ORB_UPGRADE_FROM=b8c1a23` it reports up to date, which the test now skips |
