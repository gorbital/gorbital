...
| **`--org` is refused until Phase 7** (`guard.OrgMember`), with exit 2 and a message; `orb gen resource --scope org` in an app on `gorbital.Main` too | The owner-scoped variant is right today; org scoping without its guard would duplicate v0.1's use-case checks the library is replacing |
