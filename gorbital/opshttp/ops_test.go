...
func TestOpsRoutesRequirePermissions(t *testing.T) {
	...
	for name, headers := range map[string][]string{
		...
		if r := opstest.Do(t, h, "GET", "/ops/settings", "", headers...); r.Code != http.StatusUnauthorized || r.JSON["code"] != "unauthenticated" {
			t.Errorf("%s: GET /ops/settings = %d %s, want 401 unauthenticated", name, r.Code, r.Body)
		}
	}

	noRole, _ := a.SignIn(t, "user@example.com", "")
	if r := opstest.Do(t, h, "GET", "/ops/settings", "", noRole...); r.Code != http.StatusForbidden || r.JSON["code"] != "forbidden" {
	...
	if r := opstest.Do(t, h, "GET", "/ops/settings", "", viewer...); r.Code != http.StatusOK {
		t.Errorf("GET /ops/settings as ops_viewer = %d %s, want 200", r.Code, r.Body)
	}
	if r := opstest.Do(t, h, "PUT", "/ops/settings/example.ping_message", `{"value":"hi","version":0}`, viewer...); r.Code != http.StatusForbidden {
		t.Errorf("PUT /ops/settings as ops_viewer = %d %s, want 403", r.Code, r.Body)
	}
	admin, _ := a.SignIn(t, "admin@example.com", "platform_admin")
	if r := opstest.Do(t, h, "POST", "/ops/mail/test", `{"to":"ops@example.com"}`, admin...); r.Code != http.StatusAccepted || r.JSON["delivery"] != "devmail" {
	...
	for range testEmailsPerHour - 1 {
		opstest.Do(t, h, "POST", "/ops/mail/test", `{"to":"ops@example.com"}`, admin...)
	}
	if r := opstest.Do(t, h, "POST", "/ops/mail/test", `{"to":"ops@example.com"}`, admin...); r.Code != http.StatusTooManyRequests || r.JSON["code"] != "rate_limited" {
...
func TestRuntimeSettingsThroughOps(t *testing.T) {
	...
	list := opstest.Do(t, h, "GET", "/ops/settings", "", bearer...)
	...
	r := opstest.Do(t, h, "PUT", path, `{"value":"hello","version":0,"reason":"demo"}`, append(bearer, "User-Agent", "ops-console/1")...)
	...
	// review OPS-7).
	changed := opstest.Do(t, h, "GET", "/ops/audit?action=settings.value.changed", "", bearer...)
	...
	if ping := opstest.Do(t, h, "GET", "/v1/ping", ""); ping.JSON["message"] != "hello" {
	...
	for _, tt := range errorsWanted {
		r := opstest.Do(t, h, tt.method, tt.path, tt.body, bearer...)
	...
	history := opstest.Do(t, h, "GET", path+"/history", "", bearer...)
	...
	reset := opstest.Do(t, h, "DELETE", path, `{"version":1}`, bearer...)
	...
	if ping := opstest.Do(t, h, "GET", "/v1/ping", ""); ping.JSON["message"] != "pong" {
...
func TestJobsThroughOps(t *testing.T) {
	...
	list := opstest.Do(t, h, "GET", "/ops/jobs/definitions", "", bearer...)
	...
	if r := opstest.Do(t, h, "PUT", def, `{"schedule":"@every 2h","version":0}`, bearer...); r.Code != 422 || r.JSON["code"] != "job_reason_required" {
		t.Errorf("reschedule without reason = %d %s", r.Code, r.Body)
	}
	if r := opstest.Do(t, h, "PUT", def, `{"timeout":"soon","version":0}`, bearer...); r.Code != 422 || r.JSON["code"] != "invalid_job_config" {
		t.Errorf("invalid timeout = %d %s", r.Code, r.Body)
	}
	if r := opstest.Do(t, h, "PUT", def, `{"max_attempts":500,"version":0}`, bearer...); r.Code != 422 {
	...
	// review OPS-2).
	if r := opstest.Do(t, h, "PUT", def, `{"timeout":"1s","version":0}`, bearer...); r.Code != 422 || r.JSON["code"] != "job_reason_required" {
		t.Errorf("timeout without reason = %d %s", r.Code, r.Body)
	}
	r := opstest.Do(t, h, "PUT", def, `{"schedule":"@every 2h","timeout":"30s","version":0,"reason":"less noise"}`, bearer...)
	...
	run := opstest.Do(t, h, "POST", def+"/run", "", bearer...)
	...
	opstest.WaitFor(t, "the heartbeat job to complete", func() bool {
		return opstest.Do(t, h, "GET", runPath, "", bearer...).JSON["state"] == "completed"
	})
	// On-demand runs can't pile up, and a completed run never runs again
	// (security review OPS-2, OPS-3).
	if r := opstest.Do(t, h, "POST", def+"/run", "", bearer...); r.Code != http.StatusTooManyRequests || r.JSON["code"] != "job_run_limited" {
		t.Errorf("second run within a minute = %d %s, want 429 job_run_limited", r.Code, r.Body)
	}
	if r := opstest.Do(t, h, "POST", runPath+"/retry", "", bearer...); r.Code != http.StatusConflict || r.JSON["code"] != "job_not_retryable" {
		t.Errorf("retry a completed run = %d %s, want 409 job_not_retryable", r.Code, r.Body)
	}

	runs := opstest.Do(t, h, "GET", "/ops/jobs/runs?kind=heartbeat&state=completed", "", bearer...)
	...
	if r := opstest.Do(t, h, "GET", "/ops/jobs/runs?state=exploded", "", bearer...); r.Code != 422 || r.JSON["code"] != "invalid_job_state" {
		t.Errorf("unknown state filter = %d %s", r.Code, r.Body)
	}
	if r := opstest.Do(t, h, "GET", "/ops/jobs/runs/999999999", "", bearer...); r.Code != 404 || r.JSON["code"] != "job_not_found" {
		t.Errorf("missing run = %d %s", r.Code, r.Body)
	}

	if r := opstest.Do(t, h, "PUT", def, `{"enabled":false,"version":1,"reason":"maintenance"}`, bearer...); r.Code != 200 || r.JSON["next_run_at"] != nil {
		t.Fatalf("disable = %d %s", r.Code, r.Body)
	}
	if r := opstest.Do(t, h, "POST", def+"/run", "", bearer...); r.Code != 409 || r.JSON["code"] != "job_definition_disabled" {
		t.Errorf("run disabled job = %d %s", r.Code, r.Body)
	}
	if scheduled := opstest.Do(t, h, "GET", "/ops/jobs/scheduled", "", bearer...); findDefinition(scheduled.JSON, "heartbeat") != nil {
		t.Errorf("GET /ops/jobs/scheduled = %s, want the disabled heartbeat job left out", scheduled.Body)
	}
	history := opstest.Do(t, h, "GET", def+"/history", "", bearer...)
	if changes, _ := history.JSON["changes"].([]any); len(changes) != 2 {
		t.Errorf("GET %s/history = %s, want 2 changes", def, history.Body)
	}

	opstest.WaitFor(t, "the default queue to be active", func() bool {
		queues, _ := opstest.Do(t, h, "GET", "/ops/queues", "", bearer...).JSON["queues"].([]any)
		return len(queues) == 1
	})
	// Pausing stops email delivery among everything else (security review
	// OPS-2).
	if r := opstest.Do(t, h, "POST", "/ops/queues/default/pause", "", bearer...); r.Code != 422 || r.JSON["code"] != "job_reason_required" {
		t.Errorf("pause without reason = %d %s, want 422 job_reason_required", r.Code, r.Body)
	}
	if r := opstest.Do(t, h, "POST", "/ops/queues/default/pause", `{"reason":"provider outage"}`, bearer...); r.Code != http.StatusNoContent {
		t.Errorf("pause = %d %s", r.Code, r.Body)
	}
	queues := opstest.Do(t, h, "GET", "/ops/queues", "", bearer...).JSON["queues"].([]any)
	if queues[0].(map[string]any)["paused"] != true {
		t.Errorf("queue after pause = %v", queues[0])
	}
	if r := opstest.Do(t, h, "POST", "/ops/queues/default/resume", "", bearer...); r.Code != http.StatusNoContent {
		t.Errorf("resume = %d %s", r.Code, r.Body)
	}
	paused := opstest.Do(t, h, "GET", "/ops/audit?action=jobs.queue.paused", "", bearer...)
	...
	if r := opstest.Do(t, h, "POST", "/ops/queues/reports/pause", `{"reason":"test"}`, bearer...); r.Code != 422 || r.JSON["code"] != "queue_not_active" {
