...
func TestAuditLogThroughOps(t *testing.T) {
	...
	if r := opstest.Do(t, h, "PUT", setting, `{"value":"audited","version":0,"reason":"audit demo"}`, bearer...); r.Code != http.StatusOK {
		t.Fatalf("PUT %s = %d %s", setting, r.Code, r.Body)
	}
	history := opstest.Do(t, h, "GET", setting+"/history", "", bearer...)
	...
	list := opstest.Do(t, h, "GET", "/ops/audit?action=settings.value.changed", "", bearer...)
	...
	one := opstest.Do(t, h, "GET", fmt.Sprintf("/ops/audit/%.0f", event["id"]), "", bearer...)
	if one.Code != http.StatusOK || one.JSON["action"] != "settings.value.changed" {
		t.Errorf("GET /ops/audit/{id} = %d %s", one.Code, one.Body)
	}
	byRequest := opstest.Do(t, h, "GET", "/ops/audit?request_id="+requestID, "", bearer...)
	...
	if r := opstest.Do(t, h, "DELETE", setting, `{"version":1}`, bearer...); r.Code != http.StatusOK {
		t.Fatalf("DELETE %s = %d %s", setting, r.Code, r.Body)
	}
	all := opstest.Do(t, h, "GET", "/ops/audit?actor_id="+adminID+"&limit=1", "", bearer...)
