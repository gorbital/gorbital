...
type Deps struct {
	...
	// Reauthenticate returns ctx carrying the actor that the credentials of
	// the request r (its bearer token or session cookie) authenticate now,
	// or ErrUnauthenticated when the session ended, for streams to check
	// the session while they run.
	Reauthenticate func(ctx context.Context, r *http.Request) (context.Context, error)
