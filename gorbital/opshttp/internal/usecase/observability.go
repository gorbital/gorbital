...
// authenticates the credentials of r, the request that opened the stream,
// again and checks the permission, so a signed-out or expired session, or a
// removed role, ends the stream within one interval. send returning an
// error ends the stream.
func (st *ObservabilityStream) Run(r *http.Request, send func(Overview) error) StreamEnd {
	defer st.done()
	...
	for first := true; ; first = false {
		...
		if !first {
			var err error
			if ctx, err = st.svc.authenticate(st.ctx, r); err != nil {
				if st.ctx.Err() != nil {
					return streamEnd(st.ctx)
				}
...
// authenticate returns ctx carrying the actor r's credentials authenticate,
// or ErrUnauthenticated when the session ended.
func (s *Service) authenticate(ctx context.Context, r *http.Request) (context.Context, error) {
	if s.reauthenticate == nil || r == nil {
		return nil, opsdomain.ErrUnauthenticated
	}
	return s.reauthenticate(ctx, r)
}
