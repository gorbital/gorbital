...
// of that file operates user accounts, which come with sign-in in Phase 5.
// devServer and devDo are in devoperator_test.go.

func TestOpsRateLimits(t *testing.T) {
	// Rate-limit resets need ops.auth.write, which sign-in declares for
	// platform_admin (and so for the dev console's operator); until sign-in
	// is a module (Phase 5), a stand-in module declares it as the golden
	// app's permissions did. devServer can't add modules, so the server is
	// built here.
	...
	// The golden app listed sign-in's auth_login limiter, which comes with
	// sign-in in Phase 5; an app has the built-in auth_ip limiter and the
