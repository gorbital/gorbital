...
func Mount(api huma.API, mapper *httpx.Mapper, deps Deps, modules ...Module) error {
	_, err := mount(api, mapper, deps, Scope{}, nil, modules)
	return err
}

// mount is Mount, returning what was registered, such as the rate limiters
// guards use.
func mount(api huma.API, mapper *httpx.Mapper, deps Deps, scope Scope, scopeAuth ScopeAuthorizer, modules []Module) (*registry, error) {
	if err := validateModules(modules); err != nil {
		return nil, err
	}
	reg := newRegistry(api, mapper, deps.RateLimits)
	reg.scope, reg.scopeAuth = scope.withDefaults(), scopeAuth
