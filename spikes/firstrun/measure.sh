#!/usr/bin/env bash
# Measures the Minimal first run: build orb → orb new → build app → /docs ready.
...
step "build orb (go install)" bash -c "cd '$ROOT' && go build -o '$WORK/orb' ./cmd/orb"
step "orb new my-api"          bash -c "cd '$WORK' && ./orb new my-api"
step "build app (orb dev)"     bash -c "cd '$WORK/my-api' && go build -o .orb/api ./cmd/api"

s=$(now)
(cd "$WORK/my-api" && APP_ADDR=127.0.0.1:$PORT ./.orb/api >"$WORK/server.log" 2>&1) &
PID=$!
...
(cd "$WORK/my-api" && find . -path ./.orb -prune -o -type f -print | sort)
echo "== app binary: $(ls -lh "$WORK/my-api/.orb/api" | awk '{print $5}')"
