# Spike: 3-way merge for `aps upgrade`
...
Can `aps upgrade` carry a template change into a generated file the developer has edited, without losing their edits?
...
- Otherwise run `git merge-file -p` with labelled conflict markers (`yours` / `apistock upgrade`).
- `InsertAfterAnchor` mimics `aps add` inserting a line at `//aps:anchor <name>` (text level only; the AST version is a separate spike).
...
| `aps add` inserted a module line; base and theirs rebuilt by replaying the insert | **clean merge** | no | yes |
...
4. **Replay the recorded operations to rebuild `base`.** The naive base merged cleanly here only because the inserted module line was far from the template change. Without replay, every line `aps add` inserted counts as a developer edit, so conflicts get more likely and `aps remove` can't tell generated lines from hand-written ones. `base` and `theirs` must both be rendered by replaying the operations recorded in `apistock.lock`.
5. **A conflicted file doesn't compile.** `aps upgrade` must work on a branch, stop before running `go build`, and list the conflicted files clearly.
...
- `apistock.lock` must record every applied operation (recipe, version, anchor, inserted code), not just file hashes, so `base` and `theirs` can be replayed.
- `aps upgrade` runs on a branch, reports conflicts per file, and doesn't run the build step until conflicts are resolved.
