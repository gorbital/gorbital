...
// newApp builds the app with the projects module for the test.
func newApp(t *testing.T) *gorbitaltest.App {
	t.Helper()
	return gorbitaltest.New(t,
		gorbital.WithModules(projects.Module()),
...
func TestCreateAndGetProject(t *testing.T) {
	...
	res.JSON(t, &created)
	if !strings.HasPrefix(created.ID, "prj_") || created.Name != "Website" || created.Status != "active" || created.Version != 1 {
		t.Errorf("created = %+v, want a prj_ ID, the name trimmed, the default status and version 1", created)
	}
...
// TestProjectsAreProtected checks deny by default, the permission guards and
// owner isolation: someone else's project doesn't exist for them.
func TestProjectsAreProtected(t *testing.T) {
	...
	bob.Get(item).AssertProblem(t, http.StatusNotFound, "project_not_found")
	bob.Patch(item, map[string]any{"version": 1, "name": "Mine now"}).AssertProblem(t, http.StatusNotFound, "project_not_found")
	bob.Delete(item).AssertProblem(t, http.StatusNotFound, "project_not_found")
...
func TestListProjectsPages(t *testing.T) {
	...
	ada := app.As(gorbitaltest.User("usr_ada", usecase.PermRead, usecase.PermWrite))
	ada.Post(collection, map[string]any{"name": "Website", "description": "Example description"}).AssertStatus(t, http.StatusCreated)
	ada.Post(collection, map[string]any{"name": "Docs", "description": "Example description", "status": "archived"}).AssertStatus(t, http.StatusCreated)
	...
	ada.Get(collection+"?status=archived").JSON(t, &filtered)
...
func TestUpdateAndDeleteProject(t *testing.T) {
	...
	rows, err := app.App().Deps().DB.Query(context.Background(),
		`SELECT action FROM audit_events WHERE resource_type = 'project' AND resource_id = $1 ORDER BY id`, website.ID)
	if err != nil {
		t.Fatal(err)
	}
	defer rows.Close()
