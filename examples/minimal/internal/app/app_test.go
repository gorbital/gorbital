...
func TestDocs(t *testing.T) {
	...
	disabled := newApp(t, map[string]string{"APP_DOCS_ENABLED": "false"})
...
func TestRequestIDFromTrustedCallers(t *testing.T) {
	for _, tt := range []struct {
		...
		h := newApp(t, map[string]string{"APP_TRUSTED_CALLERS": tt.callers})
...
func TestLoadConfigSecureDefaults(t *testing.T) {
	...
	if _, err := load(map[string]string{}); err == nil || !strings.Contains(err.Error(), "APP_ENV is required") {
