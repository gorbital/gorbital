...
// newAppWithMetrics builds the app from env, for the metrics listener.
func newAppWithMetrics(t *testing.T, env map[string]string) *app.App {
	t.Helper()
...
// serves HTTP and Go runtime metrics, and only /metrics, while the API
// itself doesn't serve them. Values clients choose, such as the Host header
// or path, never become labels.
func TestMetricsListener(t *testing.T) {
	a := newAppWithMetrics(t, map[string]string{"METRICS_ADDR": "127.0.0.1:0"})
	base := runMetrics(t, a)
