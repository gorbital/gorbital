...
// listener and returns its URL.
func devServer(t *testing.T, env map[string]string) string {
	t.Helper()
	full := map[string]string{"DEV_CONSOLE_TOKEN": devToken}
	for k, v := range env {
		full[k] = v
	}
	srv := httptest.NewServer(newApp(t, full))
	t.Cleanup(srv.Close)
	return srv.URL
}

// devGet requests path with the console token; headers are name/value
// pairs, and a "Host" pair sets the Host header.
func devGet(t *testing.T, base, path string, headers ...string) (int, http.Header, string) {
	...
	for i := 0; i+1 < len(headers); i += 2 {
		...
		case "Authorization":
			...
			req.Header.Set("Authorization", headers[i+1])
		default:
...
func TestDevConsole(t *testing.T) {
	...
	base := devServer(t, map[string]string{"APP_CORS_ORIGINS": "http://localhost:3000"})
	...
	if code != http.StatusOK || !strings.Contains(body, `"route":"/v1/ping"`) || !strings.Contains(body, `"path":"/v1/ping"`) {
		t.Errorf("/_dev/requests = %d %s", code, body)
	}
	if strings.Contains(body, "_dev") {
		t.Errorf("/_dev/requests lists the console's own requests: %s", body)
	}
	...
	if err := json.Unmarshal([]byte(body), &info); err != nil || code != 200 || info.Name != app.ServiceName || info.Env != "development" || len(info.Modules) == 0 {
		t.Errorf("/_dev/app = %d %s", code, body)
	}

	code, _, body = devGet(t, base, "/_dev/")
	if code != http.StatusOK || strings.Contains(body, "/_dev/mail") || strings.Contains(body, "/_dev/jobs") || !strings.Contains(body, "/_dev/config") {
		t.Errorf("minimal index = %d %s, want no mail, migrations or jobs", code, body)
	}
	if code, _, _ := devGet(t, base, "/_dev/migrations"); code != http.StatusNotFound {
		t.Errorf("/_dev/migrations = %d, want 404 in a Minimal app", code)
	}
	if code, _, body := devGet(t, base, "/_dev/openapi.json"); code != http.StatusOK || !strings.Contains(body, "/_dev/requests/stream") {
		t.Errorf("/_dev/openapi.json = %d", code)
	}
...
func TestDevConsoleSecurity(t *testing.T) {
	...
	// The app's CORS configuration doesn't reach the console.
...
func TestDevConsoleConfigHidesSecrets(t *testing.T) {
	const otlpPassword = "otlp-pass-77c1"
	cfg := testConfig(t, map[string]string{"DEV_CONSOLE_TOKEN": devToken, "OTEL_EXPORTER_OTLP_ENDPOINT": "https://user:" + otlpPassword + "@otel.example"})
	cfg.OTLPEndpoint = "" // listed, but nothing is exported in the test
	...
	defer srv.Close()
	code, _, body := devGet(t, srv.URL, "/_dev/config")
	if code != http.StatusOK {
		t.Fatalf("/_dev/config = %d %s", code, body)
	}
	for _, leak := range []string{devToken, otlpPassword} {
		if strings.Contains(body, leak) {
			t.Errorf("/_dev/config holds %q: %s", leak, body)
		}
	}
	for _, want := range []string{
		`{"name":"APP_ENV","secret":false,"set":true,"value":"development"}`,
		`{"name":"DEV_CONSOLE_TOKEN","secret":true,"set":true}`,
		`{"name":"OTEL_EXPORTER_OTLP_ENDPOINT","secret":false,"set":true,"value":"https://[redacted]@otel.example"}`,
...
func TestDevConsoleRoutes(t *testing.T) {
	...
	if handlers == 0 || !strings.Contains(body, `"path":"/v1/ping"`) {
...
func TestDevConsoleOnlyInDevelopment(t *testing.T) {
	...
	srv := httptest.NewServer(newApp(t, nil))
	defer srv.Close()
	...
	if err := load(map[string]string{"APP_ENV": "production", "DEV_CONSOLE_TOKEN_FILE": "/dev/null"}); err != nil {
		t.Errorf("production with an empty token file: error = %v", err)
	}
	...
	if err := load(map[string]string{"APP_ENV": "production"}); err != nil {
		t.Errorf("production without a token: error = %v", err)
	}
}
