...
func TestMailProviderConfiguration(t *testing.T) {
	...
	if err := load(map[string]string{"MAIL_DELIVERY": "provider"}); err == nil || !strings.Contains(err.Error(), "https://resend.com/api-keys") {
		t.Errorf("LoadConfig() without RESEND_API_KEY error = %v, want one saying where to create a key", err)
	}
	if err := load(map[string]string{"MAIL_DELIVERY": "provider", "RESEND_API_KEY": "re_123"}); err != nil {
		t.Errorf("LoadConfig() with RESEND_API_KEY error = %v", err)
	}
	if err := load(map[string]string{"RESEND_WEBHOOK_SECRET": testWebhookSecret}); err != nil {
		t.Errorf("LoadConfig() with RESEND_WEBHOOK_SECRET error = %v", err)
	}
	if err := load(map[string]string{"RESEND_WEBHOOK_SECRET": "whsec_not-base64!"}); err == nil || !strings.Contains(err.Error(), "RESEND_WEBHOOK_SECRET") || strings.Contains(err.Error(), "not-base64") {
		t.Errorf("LoadConfig() with a malformed RESEND_WEBHOOK_SECRET error = %v, want one naming the variable without quoting it", err)
	}
}
...
func TestResendWebhook(t *testing.T) {
	a := newApp(t, map[string]string{"RESEND_WEBHOOK_SECRET": testWebhookSecret})
	h := a.Handler()
