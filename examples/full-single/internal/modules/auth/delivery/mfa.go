...
type TOTPEnrollmentResponse struct {
	...
	URI    string `json:"uri" doc:"otpauth:// URI to show as a QR code. It is shown once."`
}

// RecoveryCodesResponse is a new set of recovery codes.
type RecoveryCodesResponse struct {
	RecoveryCodes []string `json:"recovery_codes" doc:"Single-use codes for signing in without the authenticator app. They are shown once; keep them somewhere safe."`
}
...
type totpCodeInput struct {
	Body struct {
		_    struct{} `json:"-" additionalProperties:"true"`
		Code string   `json:"code" maxLength:"16" example:"123456" doc:"A code from the authenticator app"`
	}
}
...
func registerMFA(api huma.API, h *handler, public, signedIn func(huma.Operation) huma.Operation) {
	...
	huma.Register(api, public(huma.Operation{
		...
		Description: "After a 202 from `POST /v1/auth/login`: send the challenge token with a code from the authenticator app or a recovery code. Each sign-in allows 5 attempts within 5 minutes; each code works once.",
	...
	huma.Register(api, signedIn(huma.Operation{
		...
		Description: "Requires the password. Returns a secret and an otpauth:// URI, shown once. Two-factor authentication turns on when a code is confirmed with `POST /v1/auth/mfa/totp/confirm`.",
		Errors:      unavailable,
	}), h.startTOTP)
	huma.Register(api, signedIn(huma.Operation{
		OperationID: "auth-confirm-totp", Method: http.MethodPost, Path: "/v1/auth/mfa/totp/confirm",
		Summary:     "Turn on two-factor authentication",
	...
	huma.Register(api, signedIn(huma.Operation{
		...
		Description:   "Requires the password and a code or recovery code; signs out other devices. Not allowed while a role of the account requires two-factor authentication.",
	...
	huma.Register(api, signedIn(huma.Operation{
		...
		Description: "Send a code from the authenticator app. Returns 10 new codes, shown once; the old codes stop working.",
		Errors:      unavailable,
...
func (h *handler) startTOTP(ctx context.Context, in *passwordInput) (*totpEnrollmentOutput, error) {
	...
	return &totpEnrollmentOutput{Body: TOTPEnrollmentResponse{Secret: e.Secret, URI: e.URI}}, nil
}
