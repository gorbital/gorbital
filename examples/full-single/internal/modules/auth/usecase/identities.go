...
// left), and revokes Apple's token. It returns ErrInvalidCredentials,
// ErrInvalidMFA, ErrIdentityNotFound or ErrLastSignInMethod.
func (s *Service) RemoveIdentity(ctx context.Context, id, password string) error {
	...
	err = s.store.InTx(ctx, func(tx Store) error {
		...
		to = u.Email
		removed, _, err = tx.DeleteIdentity(ctx, id, u.ID)
		return err
	})
	switch {
	...
	s.revokeIdentities(ctx, removed)
...
// changes are recorded. Unknown people are ignored. It returns
...
// revokeIdentities revokes the stored Apple refresh tokens of removed
// identities. Failures are logged: the identity is already unlinked.
func (s *Service) revokeIdentities(ctx context.Context, identities ...authdomain.Identity) {
	ctx = context.WithoutCancel(ctx)
	for _, i := range identities {
		if len(i.RefreshTokenCiphertext) == 0 {
			continue
		}
		p := s.providers[i.Provider]
		if p == nil || s.keyring == nil {
			s.logger.WarnContext(ctx, "can't revoke the provider's token: the provider or AUTH_ENCRYPTION_KEYS isn't configured", "provider", i.Provider, "identity_id", i.ID)
			continue
		}
		token, err := s.keyring.Decrypt(i.RefreshKeyID, i.RefreshTokenCiphertext, identityAAD(i.Provider, i.Subject))
		if err == nil {
			err = p.Revoke(ctx, string(token), i.RefreshClientID)
		}
		if err != nil {
			s.logger.ErrorContext(ctx, "revoke the provider's token", "provider", i.Provider, "identity_id", i.ID, "err", err)
		}
	}
}
