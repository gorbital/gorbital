...
const selectOAuthStateByTokenHashSQL = `
	SELECT id, token_hash, browser_hash, provider, nonce, pkce_verifier, return_to, expires_at, consumed_at, created_at
...
func (s *Store) SelectOAuthStateByTokenHash(ctx context.Context, tokenHash []byte) (authdomain.OAuthState, bool, error) {
	...
	st, err := pgx.CollectExactlyOneRow(rows, func(row pgx.CollectableRow) (authdomain.OAuthState, error) {
		...
		err := row.Scan(&st.ID, &st.TokenHash, &st.BrowserHash, &st.Provider, &st.Nonce, &st.Verifier, &st.ReturnTo, &st.ExpiresAt, &st.ConsumedAt, &st.CreatedAt)
		return st, err
	})
