...
const insertSessionSQL = `
	INSERT INTO auth_sessions (id, user_id, token_hash, created_at, last_seen_at, idle_expires_at, absolute_expires_at, ip, user_agent)
...
func (s *Store) InsertSession(ctx context.Context, in authdomain.Session) (authdomain.Session, error) {
	rows, err := s.db.Query(ctx, insertSessionSQL, in.ID, in.UserID, in.TokenHash,
		in.CreatedAt, in.LastSeenAt, in.IdleExpiresAt, in.AbsoluteExpiresAt, in.IP, in.UserAgent)
	if err != nil {
