...
// event. This is the example to copy, and the shape of modules created with
// aps gen resource (ADR-0039): change it freely.
package usecase
...
type Config struct {
	...
	// Optional.
	Logger *slog.Logger
	// Now is the clock and NewID makes project IDs, for tests.
	Now   func() time.Time
	NewID func() string
}
...
// ownerID returns the signed-in user's ID. Only users own projects; system
// and anonymous actors get ErrUnauthenticated.
...
func (s *Service) audit(ctx context.Context, action, projectID string, metadata map[string]any) {
	e := audit.Event{Action: action, ResourceType: "project", ResourceID: projectID, Outcome: audit.OutcomeSuccess, Metadata: metadata}
