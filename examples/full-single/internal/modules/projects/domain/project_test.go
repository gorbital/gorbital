...
func TestNewProject(t *testing.T) {
	...
	f.Status = ""
	project, err := domain.NewProject("prj_1", "usr_1", f, now)
	if err != nil {
		t.Fatal(err)
	}
	if project.ProjectFields != validFields() || project.ID != "prj_1" || project.OwnerID != "usr_1" || project.Version != 1 ||
...
func TestNewProjectValidates(t *testing.T) {
	...
	for _, tt := range tests {
		f := validFields()
		tt.edit(&f)
		_, err := domain.NewProject("prj_1", "usr_1", f, now)
...
func TestApply(t *testing.T) {
	project, err := domain.NewProject("prj_1", "usr_1", validFields(), now)
	if err != nil {
		t.Fatal(err)
	}
