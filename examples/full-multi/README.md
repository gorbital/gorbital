...
A Go API created with [gorbital](https://gorbital.dev) (Full preset, multi-tenant: data belongs to organisations).
...
Data belongs to organisations, and people reach it through their role in each one. Every account gets a **personal workspace** (an organisation with one owner that can't take invitations) when it is created; teams create organisations.
...
curl -X POST http://127.0.0.1:8080/v1/orgs/$ORG/invitations -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"email":"sam@example.com","role":"admin"}'  # Sam gets an email with a link (see Mailpit)
...
| Add a role for organisation members, such as a read-only viewer | `declareOrgPermissions` in `internal/app/permissions.go` |
...
| `projects` resource (`/v1/orgs/{orgId}/projects`) | Delete its permissions in `internal/app/permissions.go`, then delete `internal/modules/projects/`, `internal/app/module_projects.go` and `internal/app/projects_test.go`, its line in `internal/app/modules.go`, and the example projects in `internal/app/seed.go`. If its migration already ran, add a migration that drops the `projects` table |
