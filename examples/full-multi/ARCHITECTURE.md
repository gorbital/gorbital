...
internal/app/            composition root: builds, wires, runs and shuts down the app
  ...
  permissions.go         platform roles (platform_admin, ops_viewer), org roles (owner, admin, member) and one line per org-scoped resource (//aps:anchor org-permissions)
...
`internal/modules/orgs` owns organisations (`orgs`), their members with one role each (`org_members`) and invitations (`org_invitations`), with all four layers. The apistock orgs library supplies the organisation ID type, invitation emails and `orgs.RequireMember`.
...
`internal/modules/projects` is exactly what `aps gen resource Project name:string:unique description:text 'status:enum(active,archived)' --scope org` creates. Generate your own resources the same way, or copy it. All of it is your code: change any rule, query or response.
...
- **Lists:** `GET /v1/orgs/{orgId}/projects` uses keyset pagination through `apistock.dev/page`: `limit`, an opaque `cursor`, and `sort` by one allowlisted field, with one fixed query per sort in `repository/select_projects.go`.
