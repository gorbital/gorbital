...
const (
	orgA orgslib.ID = "org_a"
	orgB orgslib.ID = "org_b"
)

// newStore returns a store on a fresh database with the user usr_ada and the
// organisations org_a and org_b, and the database's pool.
func newStore(t *testing.T) (*projectsrepository.Store, *pgxpool.Pool) {
	...
	if _, err := pool.Exec(ctx,
		`INSERT INTO auth_users (id, email, email_normalized, created_at, updated_at) VALUES ('usr_ada', 'ada@example.com', 'ada@example.com', $1, $1)`, now); err != nil {
		t.Fatal(err)
	}
	for _, id := range []orgslib.ID{orgA, orgB} {
		if _, err := pool.Exec(ctx, `INSERT INTO orgs (id, name, created_by, created_at, updated_at) VALUES ($1, $1, 'usr_ada', $2, $2)`, id, now); err != nil {
			t.Fatal(err)
		}
	}
...
// sample returns a valid project in orgID with title as its name.
func sample(id string, orgID orgslib.ID, title string, at time.Time) projectsdomain.Project {
	return projectsdomain.Project{
		ID: id, OrgID: string(orgID), CreatedBy: "usr_ada",
...
func TestProjects(t *testing.T) {
	...
	created, err := store.InsertProject(ctx, sample("prj_1", orgA, "Website", now))
	if err != nil || created.ID != "prj_1" || created.OrgID != string(orgA) || created.CreatedBy != "usr_ada" || created.Version != 1 || !created.CreatedAt.Equal(now) {
		t.Fatalf("InsertProject() = %+v, %v", created, err)
	}
	taken := sample("prj_2", orgA, "Other", now)
	taken.Name = "WEBSITE"
	...
	if _, err := store.InsertProject(ctx, sample("prj_3", orgB, "Website", now)); err != nil {
		t.Errorf("InsertProject(same fields, another organisation) error = %v", err)
	}
	if _, err := store.InsertProject(ctx, sample("prj_4", orgA, "Docs", now)); err != nil {
		t.Fatal(err)
	}

	if p, err := store.SelectProject(ctx, orgA, "prj_1", false); err != nil || p != created {
		t.Errorf("SelectProject() = %+v, %v, want %+v", p, err, created)
	}
	if _, err := store.SelectProject(ctx, orgB, "prj_1", false); !errors.Is(err, projectsdomain.ErrProjectNotFound) {
		t.Errorf("SelectProject(another organisation's) error = %v, want ErrProjectNotFound", err)
	}
	...
	stolen := updated
	stolen.OrgID = string(orgB)
	if _, err := store.UpdateProject(ctx, stolen); !errors.Is(err, projectsdomain.ErrProjectVersionConflict) {
		t.Errorf("UpdateProject(another organisation) error = %v, want ErrProjectVersionConflict", err)
	}
	docs, err := store.SelectProject(ctx, orgA, "prj_4", false)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := store.DeleteProject(ctx, orgB, "prj_1"); !errors.Is(err, projectsdomain.ErrProjectNotFound) {
		t.Errorf("DeleteProject(another organisation's) error = %v, want ErrProjectNotFound", err)
	}
	if err := store.DeleteProject(ctx, orgA, "prj_1"); err != nil {
		t.Fatal(err)
	}
	if _, err := store.SelectProject(ctx, orgA, "prj_1", false); !errors.Is(err, projectsdomain.ErrProjectNotFound) {
		t.Errorf("deleted project selected: %v", err)
	}
	if err := store.DeleteProject(ctx, orgA, "prj_1"); !errors.Is(err, projectsdomain.ErrProjectNotFound) {
...
func TestSelectProjectsPages(t *testing.T) {
	...
	for i, title := range []string{"beta", "Alpha", "gamma", "delta", "Epsilon"} {
		p := sample(fmt.Sprintf("prj_%d", i), orgA, title, now.Add(time.Duration(min(i, 3))*time.Minute))
		if i == 4 {
	...
	if _, err := store.InsertProject(ctx, sample("prj_b", orgB, "zeta", now)); err != nil {
		t.Fatal(err)
	}
	...
	for _, tt := range tests {
		...
		for range 10 {
			items, err := store.SelectProjects(ctx, projectsusecase.ListQuery{OrgID: orgA, Status: tt.filter, Sort: tt.sort, After: after, Limit: 2})
			if err != nil {
	...
	if _, err := store.SelectProjects(ctx, projectsusecase.ListQuery{OrgID: orgA, Sort: page.SortField{Field: "org_id"}, Limit: 2}); !errors.Is(err, page.ErrInvalidSort) {
...
func TestPurgingAnOrganisationDeletesItsProjects(t *testing.T) {
	...
	ctx := context.Background()
	if _, err := store.InsertProject(ctx, sample("prj_1", orgA, "Website", now)); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(ctx, `DELETE FROM orgs WHERE id = $1`, orgA); err != nil {
		t.Fatal(err)
	}
	if _, err := store.SelectProject(ctx, orgA, "prj_1", false); !errors.Is(err, projectsdomain.ErrProjectNotFound) {
		t.Errorf("project of a purged organisation selected: %v", err)
	}
	// Projects outlive their creator's account.
	if _, err := store.InsertProject(ctx, sample("prj_2", orgB, "Docs", now)); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(ctx, `DELETE FROM auth_users WHERE id = 'usr_ada'`); err != nil {
		t.Fatal(err)
	}
	if _, err := store.SelectProject(ctx, orgB, "prj_2", false); err != nil {
		t.Errorf("project whose creator's account was purged error = %v, want it kept", err)
	}
}
...
func TestInTxRollsBack(t *testing.T) {
	...
	err := store.InTx(ctx, func(tx projectsusecase.Store) error {
		if _, err := tx.InsertProject(ctx, sample("prj_1", orgA, "Website", now)); err != nil {
			return err
		}
		return errFail
	})
