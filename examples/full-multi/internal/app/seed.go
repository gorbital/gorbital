...
// seedProjects are the example projects in the seeded administrator's
// personal workspace.
...
func Seed(ctx context.Context, cfg Config, email string, w io.Writer) error {
	...
	projects, err := projectsmodule.New(deps.pool, projectsusecase.Config{
		Recorder: deps.recorder, Catalog: deps.orgs.Catalog(), Memberships: deps.orgs.Memberships(),
	})
	if err != nil {
		return err
	}
	...
	// The examples go in the administrator's personal workspace, created with
	// the account, as if they had created them.
	workspace, err := deps.orgs.EnsurePersonalWorkspace(ctx, admin.ID)
	if err != nil {
		return fmt.Errorf("find the administrator's personal workspace: %w", err)
	}
	asAdmin := actor.With(ctx, actor.Actor{Kind: actor.KindUser, ID: admin.ID, Label: admin.Email})
	for _, p := range seedProjects {
		if _, err := projects.Service().Create(asAdmin, orgslib.ID(workspace.ID), p); err != nil {
  ...
  Recovery codes:  %s
                   %s
  Workspace:       %s (the administrator's personal workspace)
  Projects:        %d examples in it
...
`, admin.Email, password, enrollment.Secret, enrollment.URI,
		strings.Join(recoveryCodes[:half], "  "), strings.Join(recoveryCodes[half:], "  "), workspace.ID, len(seedProjects))
	return nil
}
