...
// one role; add roles here for your product, such as a read-only "viewer".
func declareOrgPermissions() *authlib.Catalog {
	...
	c.Permission(projectsusecase.PermRead, "See projects")
	c.Permission(projectsusecase.PermWrite, "Create, change and delete projects")

	member := []string{orgsusecase.PermOrgRead, orgsusecase.PermMembersRead, projectsusecase.PermRead, projectsusecase.PermWrite}
