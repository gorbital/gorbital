...
func TestProjectsEndToEnd(t *testing.T) {
	...
	collection := "/v1/orgs/" + adaOrg + "/projects"
	...
	// ...and its projects aren't in theirs.
	bobItem := "/v1/orgs/" + bobOrg + "/projects/" + id
	for _, r := range []response{
		do(t, h, "GET", bobItem, "", bob...),
		do(t, h, "PATCH", bobItem, `{"version":1,"name":"Mine now"}`, bob...),
		do(t, h, "DELETE", bobItem, "", bob...),
	} {
		if r.code != http.StatusNotFound || r.json["code"] != "project_not_found" {
			t.Errorf("request for another organisation's project = %d %s, want 404 project_not_found", r.code, r.body)
		}
	}
	...
	rows, err := pool.Query(context.Background(), `SELECT action, actor_id, org_id FROM audit_events WHERE resource_type = 'project' AND resource_id = $1 ORDER BY id`, id)
	if err != nil {
		t.Fatal(err)
	}
	defer rows.Close()
	...
	want := []string{
		"projects.project.created by ada in ada's workspace",
		"projects.project.updated by ada in ada's workspace",
		"projects.project.deleted by ada in ada's workspace",
	}
