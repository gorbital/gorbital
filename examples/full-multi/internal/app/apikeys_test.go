...
func TestAPIKeysEndToEnd(t *testing.T) {
	...
	if r := do(t, h, "POST", "/v1/auth/api-keys", body(fmt.Sprintf(`,"password":%q,"scopes":["ops.settings.read"]`, testPassword)), ada...); r.code != http.StatusUnprocessableEntity || r.json["code"] != "invalid_api_key_scopes" {
	...
	if r := do(t, h, "GET", "/v1/ping", "", "Authorization", "Bearer "+adminKey); r.code != http.StatusOK {
...
func TestServiceAccountsThroughOps(t *testing.T) {
	...
	first, _ := createKey(t, h, "/ops/service-accounts/"+otherID+"/keys", retry, withKey(admin, "create-sync-key"))
	second, _ := createKey(t, h, "/ops/service-accounts/"+otherID+"/keys", retry, withKey(admin, "create-sync-key"))
	if first == second {
	...
	if r := do(t, h, "GET", "/ops/settings", "", bearer...); r.code != http.StatusForbidden || r.json["code"] != "forbidden" {
		t.Errorf("GET /ops/settings with a service account's key = %d %s, want 403 forbidden", r.code, r.body)
	}
	...
	if r := do(t, h, "GET", "/v1/ping", "", bearer...); r.code != http.StatusOK {
		t.Errorf("GET /v1/ping with a service account's key = %d", r.code)
	}
