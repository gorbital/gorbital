...
type systemReporter struct {
	...
	tracker *releases.Tracker
	started time.Time
	workers int
}

// systemTimeout bounds each database call the report makes.
...
func (r systemReporter) SystemReport(ctx context.Context) opsusecase.SystemReport {
	...
	report := opsusecase.SystemReport{
		Instance: opsusecase.SystemInstance{
			ID: r.tracker.InstanceID(), Version: info.Version, Commit: info.Commit, BuildTime: info.BuildTime,
			Modified: info.Modified, StartedAt: r.started.UTC(), Uptime: time.Since(r.started),
		},
		Database: r.database(ctx),
		Runtime:  goRuntime(),
		Jobs:     opsusecase.SystemJobs{Workers: r.workers},
	}
	if r.workers > 0 {
		report.Jobs.Queues = []string{river.QueueDefault}
	}

	status, _ := r.health.Check(ctx)
...
func (r systemReporter) database(ctx context.Context) opsusecase.SystemDatabase {
	...
	state, err := postgres.Migrations(migrateCtx, r.pool, migrations.FS)
	if err != nil {
