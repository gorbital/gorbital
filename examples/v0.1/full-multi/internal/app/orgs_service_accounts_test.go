...
func TestOrganisationsCantReachEachOthersServiceAccounts(t *testing.T) {
	...
	if r := do(t, h, "POST", "/v1/invitations/accept", fmt.Sprintf(`{"token":%q}`, emailedInvitation(t, pool, "bob@example.com")), bob...); r.code != http.StatusOK {
	...
	noSecretsStored(t, dbURL, key, readOnly, ownerKey, fresh)
}
