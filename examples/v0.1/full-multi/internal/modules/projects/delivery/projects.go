...
// operations and request and response types for /v1/orgs/{orgId}/projects.
package delivery
...
type createInput struct {
	...
	Body  struct {
		...
		Name        string   `json:"name" maxLength:"100" doc:"Unique in the organisation, ignoring case"`
...
func Register(api huma.API, svc *projectsusecase.Service) {
	...
	huma.Register(api, inOrg(huma.Operation{
		OperationID: "projects-create", Method: http.MethodPost, Path: "/v1/orgs/{orgId}/projects",
		Summary: "Create a project", DefaultStatus: http.StatusCreated,
		Errors: []int{http.StatusConflict, http.StatusUnprocessableEntity},
	}), h.create)
	huma.Register(api, inOrg(huma.Operation{
		OperationID: "projects-list", Method: http.MethodGet, Path: "/v1/orgs/{orgId}/projects",
		Summary:     "List the organisation's projects",
	...
	huma.Register(api, inOrg(huma.Operation{
		OperationID: "projects-get", Method: http.MethodGet, Path: "/v1/orgs/{orgId}/projects/{id}",
		Summary: "Get a project",
	}), h.get)
	huma.Register(api, inOrg(huma.Operation{
		OperationID: "projects-update", Method: http.MethodPatch, Path: "/v1/orgs/{orgId}/projects/{id}",
		Summary:     "Update a project",
		Description: "Send the fields to change and the `version` you read.",
		Errors:      []int{http.StatusConflict, http.StatusUnprocessableEntity},
	}), h.update)
	huma.Register(api, inOrg(huma.Operation{
		OperationID: "projects-delete", Method: http.MethodDelete, Path: "/v1/orgs/{orgId}/projects/{id}",
		Summary: "Delete a project", DefaultStatus: http.StatusNoContent,
	}), h.delete)
}

func (h *handler) create(ctx context.Context, in *createInput) (*projectOutput, error) {
	p, err := h.svc.Create(ctx, orgslib.ID(in.OrgID), projectsdomain.ProjectFields{
		Name:        in.Body.Name,
...
func (h *handler) list(ctx context.Context, in *listInput) (*projectPageOutput, error) {
	res, err := h.svc.List(ctx, orgslib.ID(in.OrgID), projectsusecase.ListInput{
		Page:   in.Params,
...
func (h *handler) get(ctx context.Context, in *idInput) (*projectOutput, error) {
	p, err := h.svc.Get(ctx, orgslib.ID(in.OrgID), in.ID)
	if err != nil {
		return nil, err
	}
	return &projectOutput{Body: projectResponse(p)}, nil
...
func (h *handler) update(ctx context.Context, in *updateInput) (*projectOutput, error) {
	...
	p, err := h.svc.Update(ctx, orgslib.ID(in.OrgID), in.ID, projectsusecase.UpdateInput{Version: in.Body.Version, Changes: changes})
	if err != nil {
		return nil, fieldErrors(err, "body")
	}
	return &projectOutput{Body: projectResponse(p)}, nil
...
func (h *handler) delete(ctx context.Context, in *idInput) (*struct{}, error) {
	return nil, h.svc.Delete(ctx, orgslib.ID(in.OrgID), in.ID)
}
