...
type Project struct {
	ID string
	// OrgID is the organisation's ID (orgs.ID in the other layers).
	OrgID string
	// CreatedBy is the member who created it, for display and audit.
	CreatedBy string
	ProjectFields
...
// NewProject returns a new project in orgID created by createdBy, or a
...
func NewProject(id, orgID, createdBy string, f ProjectFields, now time.Time) (Project, error) {
	...
	p := Project{ID: id, OrgID: orgID, CreatedBy: createdBy, ProjectFields: f, Version: 1, CreatedAt: now, UpdatedAt: now}
