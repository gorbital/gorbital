...
type appSettings struct {
	...
	authAPIKeyMaxTTL            *settings.Setting[time.Duration]
	authAPIKeyFailures          *settings.Setting[int]
...
func declareSettings(reg *settings.Registry) appSettings {
	return appSettings{
		...
		// API keys (ADR-0058).
		authAPIKeyMaxTTL: settings.Duration(reg, "auth.api_key_max_ttl", authlib.DefaultAPIKeyMaxTTL,
			settings.Describe("The longest lifetime of a new API key. Every key needs an expiry within it; existing keys keep theirs."),
			settings.Range(24*time.Hour, 365*24*time.Hour),
			settings.ReasonRequired(),
		),
		authAPIKeyFailures: settings.Int(reg, "auth.api_key_failures_per_minute", authusecase.DefaultAPIKeyFailures,
