...
// selectShelvesSQL holds one fixed query per sort, keyed "created_at",
...
var selectShelvesSQL = func() map[string]string {
	...
	for _, s := range sorts {
		for _, desc := range []bool{false, true} {
			...
			queries[name] = `
	SELECT ` + shelfColumns + ` FROM shelves
	WHERE owner_id = $1
	  AND ($6::text = '' OR visibility = $6)
...
// SelectShelves returns up to q.Limit of an owner's shelves in q.Sort
// order, starting after q.After.
func (s *Store) SelectShelves(ctx context.Context, q usecase.ListQuery) ([]domain.Shelf, error) {
	name := q.Sort.Field
	if q.Sort.Desc {
		name = "-" + name
	}
	sql, ok := selectShelvesSQL[name]
	if !ok {
	...
	rows, err := s.db.Query(ctx, sql, q.OwnerID, q.After != nil, after, afterID, q.Limit, q.Visibility)
	if err != nil {
		return nil, err
	}
	return pgx.CollectRows(rows, scanShelf)
}
