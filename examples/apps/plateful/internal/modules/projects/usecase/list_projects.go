...
// listOptions are the page sizes and sorts ListProjects accepts.
...
// ListProjectsInput selects a page of an organisation's projects.
...
// ListProjects returns a page of the organisation orgID's projects, sorted by
// one field (newest first by default).
func (s *Service) ListProjects(ctx context.Context, orgID string, in ListProjectsInput) (page.Result[domain.Project], error) {
	var none page.Result[domain.Project]
	...
	if in.Status != "" && !in.Status.Valid() {
		return none, &domain.ValidationError{Errors: []domain.FieldError{{Field: "status", Message: "must be active or archived"}}}
	}
	...
	res := page.Result[domain.Project]{Items: items}
