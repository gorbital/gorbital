// Package orders is the orders module: orders that belong to an
...
// Module returns the orders module. main.go adds it with every other
...
func Module() gorbital.Module {
	return gorbital.Module{
		...
		Errors: []httpx.Mapping{
			...
			{Err: domain.ErrOrderNotFound, Status: http.StatusNotFound, Code: "order_not_found", Detail: "the organisation has no order with this ID"},
			{Err: domain.ErrOrderVersionConflict, Status: http.StatusConflict, Code: "order_version_conflict", Detail: "the order changed since you read it; get it again and retry"},
		},
		...
		Permissions: []gorbital.Permission{
			{Name: usecase.PermRead, Description: "See the organisation's orders", OrgRoles: []string{"owner", "admin", "member"}},
			{Name: usecase.PermWrite, Description: "Create, change and delete the organisation's orders", OrgRoles: []string{"owner", "admin", "member"}},
		},
